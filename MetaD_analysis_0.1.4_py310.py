...
__date__ = "20171011"
__python3_port__ = "20260701"

import os
import numpy as np
import matplotlib as mpl

# Use a non-interactive backend automatically on headless machines.
# This keeps PDF generation usable on clusters without X forwarding.
...
class FileParser():
    ...
    def lastline(fname):
        ...
        with open(fname, 'rb') as myfile:  # Fast path for getting the last line in long files
            with closing(mmap.mmap(myfile.fileno(), 0, access=mmap.ACCESS_READ)) as mm:
                startofline = mm.rfind(b'\n', 0, len(mm) - 1) + 1
                return mm[startofline:].rstrip(b'\r\n').decode("utf-8")
    ...
    def getinfo(fname):
        ...
        with open(fname,"r") as f:
            trig = True
            comments ={}
            while trig is True: #Reads until no more lines with #!
    ...
    def multithreader(self,l_processes,n_threads): #A default mulitrheader
        ...
        class updatedThread(threading.Thread):
            def __init__(self, threadID, name, q):
                ...
                self.q = q
            def run(self):
                print("Starting T%s" % self.name)
                process_data(self.name, self.q)
                print("Exiting T%s" % self.name)
    ...
    def rad_gyr_CA(self,gro,xtc,sel="name CA",stride=1):
        ...
        try:
            ...
            try:
                ...
                toplot = (np.array((time,radius)),("Time[ns]","Rg[Å]"),"Rg("+sel+")") #Plot is added as touple (data,label,title)
...
class Plotter(): #Used to plot...
    ...
    def __init__(self,ntotplot,**kwargs):
        ...
        self.plotno= 1
        if "nrows" in kwargs and "ncols" in kwargs:  #Setting up the plot grid
    ...
    def twoD3Dplot(self, fn_fes, remove=0,rem_dir='x',xlabel=None,ylabel=None,**kwargs):
        ...
        # Get rid of the panes/spines. Matplotlib renamed w_xaxis/w_yaxis/w_zaxis
        # to xaxis/yaxis/zaxis in newer releases.
    ...
    def twoDcontourplot(self,feslist,CVs=None,zeroset = True,**kwargs):
        ...
        def update(val):
            for idx,sl in enumerate(self.contfsliders):
                ...
                target_ax = getattr(self.contax[idx], "axes", getattr(self.contax[idx], "ax", None))
                self.contax[idx] = target_ax.contour(xr, yr, Zdata, 11,
                                                      vmin=Zmin, vmax=Zmax,
                                                      cmap=mpl.cm.jet_r, **kwargs)
        
        divider = make_axes_locatable(plt.gca())
        sloc = divider.append_axes("top","5%",pad="1%")
        sloc.set_facecolor('lightgoldenrodyellow')
        self.axtime.append(sloc)
    ...
    def oneDfesplot(self,feslist,CV,time=None,zeroset=True,**kwargs):
        ...
        divider = make_axes_locatable(plt.gca())
        ax.set_prop_cycle(color=[colormap(i) for i in np.linspace(0.0, 1, len(feslist))])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-rg",action = "store_true", help = "Plot Rg from .xtc and .gro file")
    parser.add_argument("-xtc",metavar="fname.xtc", type = str, help="specify .xtc file")
    parser.add_argument("-gro",metavar="fname.gro", type = str, help="specify .gro file")
    ...
    if os.path.isfile(args.plumed) is False and args.n is False: #Does plumed exist here?
        raise OSError("Plumed not found on location: %s"%args.plumed)
   

    if args.plot is True: #Is user running with a display?
        try:
            os.environ['DISPLAY']
        except KeyError:
    ...
    if args.nocolvar is False: #CVs in colvar, added to plotlist
    ...
    if args.rg is True: #Runs MDAnalysis Rg calculation, adds to plotlist, saves data in bashrunner
        if args.gro is None or args.xtc is None:
            raise ValueError("Both -gro and -xtc must be specified when using -rg")
        bRun.rad_gyr_CA(args.gro,args.xtc,stride=10)

     
    if args.nohills is False: #Hillsheight added to plotlist
...
#    some.twoD3Dplot("./2D_ab1_rg_ca/fes_5.dat")
    ...
    if args.plot is True: #Show the plot in the display
        for fig_runner in (some, some2, some3):
            if fig_runner is not None:
                fig_runner.show()
    ...
    if args.pdf is not None: #Print as a pdf
        ...
        if some2 is not None:
            some2.writePdf(fn_pdf=pdf+"_1D"+ext)
        if some3 is not None:
